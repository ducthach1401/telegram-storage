...
- **`@nestjs/schedule`** — backup MySQL định kỳ → cùng kênh `TELEGRAM_STORAGE_CHAT_ID`, thư mục ảo (mặc định `backup`) — tuỳ chọn `MYSQL_BACKUP_ENABLED=true`
...
| `MYSQL_BACKUP_ENABLED` | `true` — chạy cron `mysqldump` + lưu `.sql.gz` như file drive trên kênh `TELEGRAM_STORAGE_CHAT_ID` |
...
| `POST` | `/api/v1/telegram/webhook` | BotFather `setWebhook` — đồng bộ document gửi vào `TELEGRAM_STORAGE_CHAT_ID` vào DB (**public**; tuỳ chọn header secret) |
...
- **Đồng bộ ngược Telegram:** `POST /api/v1/telegram/webhook` — nên đặt `TELEGRAM_WEBHOOK_SECRET` và cấu hình cùng secret khi `setWebhook`. File lưu vào `TELEGRAM_SYNC_FOLDER_ID` hoặc root nếu để trống. Tin không có `document` hoặc chat khác `TELEGRAM_STORAGE_CHAT_ID` bị bỏ qua (trả `200` không xử lý).
